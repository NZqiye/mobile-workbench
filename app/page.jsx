...
function mergeConsultationRecord(base, incoming) {
  const baseUpdatedAt = itemUpdatedAt(base);
  const incomingUpdatedAt = itemUpdatedAt(incoming);
  const newer = incomingUpdatedAt > baseUpdatedAt ? incoming : base;
  ...
  return {
    ...older,
    ...newer,
    status: status || newer.status || older.status,
    seasonProgress: { ...(older?.seasonProgress || {}), ...(newer?.seasonProgress || {}) },
    seasonRatings: { ...(older?.seasonRatings || {}), ...(newer?.seasonRatings || {}) },
    seasonEpisodeCounts: { ...(older?.seasonEpisodeCounts || {}), ...(newer?.seasonEpisodeCounts || {}) },
    updatedAt: Math.max(baseUpdatedAt, incomingUpdatedAt),
  };
}
...
function mergeTmdbFields(item, fresh) {
  return {
    ...
    tmdbRating: fresh.tmdbRating ?? item.tmdbRating ?? "",
...
const subscriptionIconCategories = [{
  ...
  icons: [
    ...
    "thiings-subscribe:netease-music",
    "thiings-subscribe:reading",
    "thiings-subscribe:openai",
    "thiings-subscribe:anthropic",
    "thiings-subscribe:deepseek",
    "thiings-subscribe:quark-drive",
    "thiings-subscribe:clash",
    "thiings-subscribe:clouddrive",
    "thiings-subscribe:book",
    "thiings-subscribe:goodshort-short-dramas-hub",
    "thiings-subscribe:amazon-prime-video",
    "thiings-subscribe:hbo-max-programas-e-filmes",
    "thiings-subscribe:tencent-video",
    "thiings-subscribe:bilibili",
    "thiings-subscribe:iqiyi",
    "thiings-subscribe:youku",
    "thiings-subscribe:douyin",
  ],
}];
...
function WatchCheckin({ items = [], tmdbResults = [], tmdbStatus = "", onSearchTmdb, onImportTmdb, onCheckin, onSyncTmdbRating, onRemoveCheckin, onRefreshCloud, syncStatus = "", watchCheckins = [] }) {
  ...
  const mobileLibraryVisible = mobileLibraryOpen || watchItems.length === 0;
    ...
    <section className={`watch-rating-panel ${mobileLibraryVisible ? "mobile-library-open" : ""}`}>
      ...
      {watchItems.length === 0 ? <p className="empty">先搜索或添加一部影视，再回来记录。</p> : (
        <>
          ...
          <button className="watch-rating-mobile-picker-toggle" type="button" onClick={() => setMobileLibraryOpen((value) => !value)}>
            {mobileLibraryOpen ? "收起片单" : `更换作品 · ${detailTitle}`}
          </button>
          <div className="watch-rating-layout">
            ...
            {selected && <div className="watch-rating-detail">
              ...
              <div className="watch-rating-detail-body watch-rating-desktop-body">
              ...
              <div className="watch-rating-mobile-body">
                ...
                {isMovie ? (
                  ...
                  <>
                    {episodes.find((episode) => episode.airDate && episode.airDate >= todayKey()) && <div className="watch-mobile-next">
                      {(() => {
                        const nextEpisode = episodes.find((episode) => episode.airDate && episode.airDate >= todayKey());
                        return nextEpisode.stillUrl || detailPoster ? <img src={nextEpisode.stillUrl || detailPoster} alt="" /> : <span className="watch-mobile-next-placeholder">E{nextEpisode.episodeNumber}</span>;
                      })()}
                      {(() => {
                        const nextEpisode = episodes.find((episode) => episode.airDate && episode.airDate >= todayKey());
                        return <div><small>下一集 · {nextEpisode.airDate}</small><strong>{nextEpisode.name || "下一集"}</strong><span>S{seasonNumber} E{nextEpisode.episodeNumber}</span></div>;
                      })()}
                    </div>}
                    <div className="watch-mobile-section-title"><strong>季</strong><span>{seasonList.length} 季</span></div>
